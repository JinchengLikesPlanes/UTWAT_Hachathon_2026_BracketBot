...
export function buildTable(THREE, view) {
  ...
  for (const x of [0.15, R.TABLE_LEN - 0.15]) for (const z of [-0.6, 0.6]) { const leg = new THREE.Mesh(new THREE.BoxGeometry(0.05, TABLE_H, 0.05), new THREE.MeshStandardMaterial({ color: 0x2a2724 })); leg.position.set(x, TABLE_H / 2, z); g.add(leg) }
