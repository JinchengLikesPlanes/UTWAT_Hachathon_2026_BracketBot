...
const lessonMeta = {
  pid: { number: '01', title: 'Keep it steady', subtitle: 'Tune a PID controller', color: '#ee7b48' },
  rl: { number: '02', title: 'Teach a return', subtitle: 'Train a ping-pong policy', color: '#5bb5a4' },
  vision: { number: '03', title: 'Teach it colors', subtitle: 'Build a visual classifier', color: '#eab84d' },
}
...
function Topbar({ lesson, back }: { lesson: LessonId; back: () => void }) {
  ...
  return <header className="topbar">
    ...
    <div className="brand"><span className="brand-mark">B]</span> BRACKETBOT LAB</div>
    <div className="lesson-chip" style={{ '--accent': meta.color } as React.CSSProperties}><b>{meta.number}</b> {meta.title}</div>
  </header>
}

function Home({ progress, open }: { progress: Record<LessonId, number>; open: (id: LessonId) => void }) {
  return <main className="home">
    <header className="home-head"><div className="brand"><span className="brand-mark">B]</span> BRACKETBOT LAB</div><span className="cohort">ROBOTICS · LEVEL 01</span></header>
    <section className="hero">
      <div className="hero-copy"><p className="eyebrow">LEARN BY EXPERIMENTING</p><h1>Make a robot<br/><em>behave.</em></h1><p>Three hands-on labs. Change one thing, run the robot, and discover what makes intelligent machines work.</p><div className="hero-rule"><span>3 LABS</span><span>~50 MINUTES</span><span>NO CODE NEEDED</span></div></div>
      <div className="hero-orbit"><div className="orbit-ring"/><div className="bot-glyph">⌁<span>BRACKET<br/>BOT</span></div><i>01</i><i>02</i><i>03</i></div>
    </section>
    <section className="lesson-grid">
      {(Object.keys(lessonMeta) as LessonId[]).map((id, index) => { const item = lessonMeta[id]; const done = progress[id] >= 5; return <button className="lesson-card" key={id} onClick={() => open(id)} style={{ '--accent': item.color } as React.CSSProperties}>
        <div className="card-number">{item.number}<span>{done ? 'COMPLETE' : progress[id] ? `STEP ${progress[id] + 1} OF 5` : 'READY'}</span></div>
        <div className={`card-visual visual-${id}`}><span>{id === 'pid' ? '∿' : id === 'rl' ? '↗' : '●'}</span></div>
        <p>{id === 'pid' ? 'CONTROL' : id === 'rl' ? 'LEARNING' : 'VISION'}</p><h2>{item.title}</h2><small>{item.subtitle}</small>
        <div className="card-progress"><span style={{ width: `${Math.min(progress[id], 5) * 20}%` }}/></div><strong>{index === 0 ? 'START LAB' : 'OPEN LAB'} →</strong>
      </button> })}
    </section>
  </main>
}
...
function PIDLesson({ stage, advance }: { stage: number; advance: (n: number) => void }) {
  ...
  return <LessonLayout rail={<StageRail names={['See the error','Tune P','Add D','Add I','Challenge']} current={current} set={setCurrent}/>}>
    ...
    <aside className="control-panel"><p className="eyebrow">STEP {current + 1} / 5</p><h1>{descriptions[current][0]}</h1><p>{descriptions[current][1]}</p>
      ...
      <button className="primary" onClick={run} disabled={loading}>{loading ? 'RUNNING MUJOCO…' : 'RUN EXPERIMENT'} <span>▶</span></button>{error && <p className="error">{error}</p>}
...
function RLLesson({ stage, advance }: { stage:number; advance:(n:number)=>void }) {
  ...
  return <LessonLayout rail={<StageRail names={['Set the task','Senses & actions','Rewards','Train','Evaluate']} current={current} set={setCurrent}/>}>
    ...
    <aside className="control-panel"><p className="eyebrow">STEP {current+1} / 5</p><h1>{copy[0]}</h1><p>{copy[1]}</p>
...
function VisionLesson({stage,advance}:{stage:number;advance:(n:number)=>void}) {
  ...
  const tryOwn=async()=>{if(!model)return;setError('');try{const rgb=[0,1,2].map(i=>parseInt(own.slice(1+i*2,3+i*2),16));const r=await post<Prediction>('/api/vision/predict',{rgb,...model});setOwnResult(`${r.label} · ${Math.round(r.scores[r.label]*100)}%`)}catch(e){setError((e as Error).message)}}
  ...
  return <LessonLayout rail={<StageRail names={['Label','Train','Test','Improve','Try your own']} current={current} set={setCurrent}/>}>
    ...
    <aside className="control-panel"><p className="eyebrow">STEP {current+1} / 5</p><h1>{copy[0]}</h1><p>{copy[1]}</p>
      ...
      {current===1&&<><button className="primary" onClick={()=>train(false)} disabled={busy||samples.length<6}>{busy?'TRAINING…':'TRAIN CLASSIFIER'} <span>▶</span></button><p className="micro">Need at least two examples of each color.</p></>}
...
function LessonLayout({rail,children}:{rail:React.ReactNode;children:React.ReactNode}) { return <><div className="lesson-body"><div className="rail-wrap">{rail}</div><div className="lesson-content">{children}</div></div><footer className="lab-footer"><span>SIMULATION MODE</span><span>Experiments stay on this computer</span><span>?</span></footer></> }
